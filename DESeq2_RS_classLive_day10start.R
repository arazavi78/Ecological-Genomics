...
countsTableRound <- round(day10countstable) 
...
res_interClimTreat <- results(dds, name="treatment_D_vs_C", alpha=0.05)
...
guz <- results(dds, contrast=c("treatment","H","D"), alpha=0.05)
head(guz)

summary(guz)

topgenes <- head(rownames(guz),20)
mat <- assay(vsd)[topgenes,]
mat <- mat - rowMeans(mat)
df <- as.data.frame(colData(dds)[,c("treatment","climate")])
pheatmap(mat, annotation_col=df)
